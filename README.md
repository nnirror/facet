...
To improve syntax and legibility, there is a shorthand format for creating a new instance of a `FacetPattern`. The rest of this document will use this shorthand.

`new FacetPattern() == _;`

A FacetPattern has no data when it initializes, so first, we need to generate and attach some data. (The `FacetPattern generators` section of the command reference below has a list of all generators).
...
`_.from([1,1,2,3]).palindrome(); // outputs 1 1 2 3 3 2 1 1`
...
- **chaos** ( _length_, _x_, _y_)
	- over _length_ iterations, computes `x = (x*x)+y`. Returns the series of all `x` values computed over time. NOTE: iterative functions can have both stable and unstable results ( hello... Mandelbrot set... :D ). However in a musical context, there is not much use for an unstable result which would endlessly grow to infinity. So this function clips between -1 and 1, and there is no danger of outputting humongous numbers. For stable results, the best x values are within -0.8 and 0.25, and the best y values are within -0.8 and 0.8.
	- example:
...
- **get** ( _name_ )
	- retrieves a FacetPattern previously stored in memory by a `.set()` command. **NOTE**: You cannot run `.get()` in the same block of commands where the pattern was initially stored via `.set()`.
		- example:
		- `_.set('my_pattern'); // first, set a random sample, in a separate command`
...
- **saveAs** ( _filename_ )
	...
	- example:
		- `_.iter(6,1,()=>{this.append(_.sine(random(1,40,1),100)).saveAs(\`/myNoiseStuff/${Date.now()}\`)}); // creates 6 wav files in the myNoiseStuff directory. Each filename is the UNIX timestamp`
---
...
---
- **skip** ( _prob_ )
	- Sometimes, skip executing the command, as if it had never been attempted. Useful if you only want to update the wavetable some of the time, but otherwise want to preserve the previously generated wavetable.
		- example:
...
- **add** ( _FacetPattern_ )
	...
	- example:
		- `_.sine(1,100).add(data([0.5, 0.25, 0.1, 1]));`
- **and** ( _FacetPattern_ )
...
---
- **mix** ( _wet_, _FacetPattern_ )
	- mixes the input FacetPattern with a second FacetPattern. `wet = 1` would be the second FacetPattern entirely, with no dry signal. `wet = 0` would be only the dry signal, with none of the second FacetPattern.
	- example:
		- `_.randsamp().mix(0.5,()=>{this.speed(0.2).echo(6)}).play(); // weird lil drum`
---
...
- **or** ( _FacetPattern_ )
	...
	- example:
		- `_.from([1,0,1,0]).or(data([0,1])); // 1 0 1 1`
---
...
Facet can load audio samples (currently only .wav files) as FacetPatterns and run arbitrary operations on them.
 ...
 ---
- **audio** ( )
	- cancels any DC offset by running the pattern through a high-pass filter at ~0Hz, which can be useful when preparing a synthesized FacetPattern for audio output.
	- example:
